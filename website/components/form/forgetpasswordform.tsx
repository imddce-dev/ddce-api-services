...
/** ปรับแต่งได้ผ่าน Props */
export type ForgotPasswordProps = {
    /** พาหนีไปหน้าหลังสำเร็จ */
    redirectPath?: string;
    /** เวลานับถอยหลัง (วินาที) */
    autoRedirectSeconds?: number;
    /** ถ้าส่งมา จะใช้ฟังก์ชันนี้แทนการเรียก /api/auth/forgot-password */
...
export default function ForgotPasswordForm({
    ...
    // สำหรับยกเลิก request เมื่อ unmount/กดซ้ำ
    ...
    async function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        if (!emailOk || phase === "sending") return;
    ...
    const Wrapper: React.FC<React.PropsWithChildren> = ({ children }) =>
        frameless ? (
            ...
            <section className="rounded-2xl border border-slate-700/70 bg-slate-900/60 backdrop-blur-2xl shadow-xl">
                {children}
            </section>
        );
    ...
    return (
        <div className="relative mx-auto w-[min(650px,94vw)]">
            <Wrapper>
                ...
                <div className={frameless ? "" : "px-6 pb-6"}>
                    {phase === "done" ? (
                        <SuccessPanel
                            email={email}
                            countdown={countdown}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4 pt-2" noValidate>
                            {/* ==== Email field: สไตล์เดียวกับ LoginForm ==== */}
                            <div>
                                <label
                                    htmlFor="email"
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className={`w-full rounded-lg border bg-slate-900/80 pl-10 pr-4 py-2 text-slate-100 outline-none transition duration-700 ease-in-out focus:ring-2 ${email.length > 0 && !emailOk
                                ...
                                <Transition
                                    ...
                                    leaveTo="opacity-0"
                                >
                                    <p id="email-help" className="mt-1 text-[12px] font-bold text-red-400">
                                        รูปแบบอีเมลไม่ถูกต้อง
                                    </p>
                                </Transition>
                            </div>

                            {/* ปุ่มส่ง: โทนเดียวกับ LoginForm */}
                            <button
                                type="submit"
                                disabled={!emailOk || phase === "sending"}
                    ...
                    {/* ลิงก์กลับหน้าเข้าสู่ระบบ: โทนเดียวกัน */}
...
/* ===== Sub-components (คงเดิม ปรับปุ่มให้โทนเดียว) ===== */
    ...
    return (
        ...
        >
            <div className="flex items-start gap-1">
                <CheckCircle2 className="mt-0.5 h-1 w-5 text-emerald-300" />
            ...
            <div className="grid gap-2 rounded-xl border border-white/10 bg-white/5 p-3">
                <div className="flex items-center justify-between text-xs text-slate-200">
                    <span className="inline-flex items-center gap-1">
                        ...
                        {autoRedirect ? (
                            <>
                                กำลังกลับสู่หน้าล็อกอินใน{" "}
                                <b className="tabular-nums">{countdown}s</b>
                            </>
                        ) : (
